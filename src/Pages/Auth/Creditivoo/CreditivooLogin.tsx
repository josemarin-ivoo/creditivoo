...
  const [showPasswordInput, setShowPasswordInput] = useState(false);
  ...
  const checkBiometricSupportAndCredentials = async () => {
    try {
      const supported = await Keychain.getSupportedBiometryType() as string; 
    
    const hasCredentials = await Keychain.hasGenericPassword({ service: BIOMETRIC_SERVICE });

      // Solo mostramos el botón si hay soporte y ya hay credenciales guardadas para esta app
      if (supported !== 'None' && hasCredentials) { 
  ...
  const handleContinue = async () => {
    ...
    // 1. Guardar la preferencia de 'Recuérdame'
    await saveRememberMePreference(email); 

    // 2. Muestra el campo de contraseña
    setShowPasswordInput(true); 
    
    // Si la biometría está disponible, ocultamos el botón de huella temporalmente para simplificar la UI
    // (Opcional, podrías dejarlo si quieres que el usuario pueda cambiar a biometría desde la clave)
    // setShowBiometricButton(false); 
};
